...
export default function LichHocManagement({ user }) {
  ...
  const fetchLichHocs = async () => {
    try {
      ...
      if (user?.role === "admin") response = await lichHocAPI.getAll();
        ...
        response = await lichHocAPI.getByGiaSu(userId);
  ...
  const getStatusInfo = (status, ngay_hoc, gio_ket_thuc, role) => {
    ...
    if (ngay_hoc && gio_ket_thuc && checkIsOverdue(ngay_hoc, gio_ket_thuc)) {
      ...
      if (role !== "gia_su")
        return { label: "Chờ gia sư chốt", color: "bg-gray-100 text-gray-600" };
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.tao_chu_ky) {
      ...
      if (formData.ngay_trong_tuan.length === 0) {
        setAlertModal({
          show: true,
          message:
            "Vui lòng chọn ít nhất 1 ngày trong tuần để xếp lịch định kỳ!",
          type: "error",
        });
        return;
      }
      ...
      for (let day of formData.ngay_trong_tuan) {
        ...
        if (!time || !time.gio_bat_dau || !time.gio_ket_thuc) {
          setAlertModal({
            show: true,
            message: "Vui lòng nhập đầy đủ giờ học cho tất cả các Thứ đã chọn!",
            type: "error",
          });
          return;
        }
        if (time.gio_bat_dau >= time.gio_ket_thuc) {
          setAlertModal({
            show: true,
            message: "Giờ kết thúc phải lớn hơn Giờ bắt đầu ở tất cả các ngày!",
            type: "error",
          });
          return;
        }
    ...
    try {
      if (editingId) {
        ...
        setAlertModal({
          show: true,
          message: formData.tao_chu_ky
            ? "Hệ thống đã tự động tạo lịch chu kỳ thành công!"
            : "Tạo lịch học mới thành công!",
          type: "success",
        });
      }
      setShowModal(false);
  ...
  const handleChotCaClick = (id) => {
    setConfirmModal({
      show: true,
      message:
        "Xác nhận hoàn thành ca dạy này? (Sau đó bạn có thể chuyển sang phần Điểm danh học sinh)",
      onConfirm: async () => {
        try {
          ...
          setAlertModal({
            show: true,
            message: "Có lỗi xảy ra khi chốt ca!",
            type: "error",
          });
        }
      },
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 relative">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 border-b pb-4 gap-4">
          ...
          <div className="flex items-center gap-2 bg-gray-50 p-1.5 rounded-lg border border-gray-200">
            ...
            <button
              onClick={goToToday}
              className="px-4 py-1.5 text-sm font-medium hover:bg-white hover:shadow-sm rounded text-gray-700 transition-all"
            >
              Tuần này
            </button>
            <button
          ...
          {weekDays.map((dayDate, idx) => {
            ...
            return (
              <div key={dateStr} className="text-center">
                <div
                  className={`mb-3 pb-2 border-b-2 ${isToday ? "border-blue-500" : "border-transparent"}`}
                >
                  <p
                    className={`font-bold ${isToday ? "text-blue-600" : "text-gray-700"}`}
                  >
                    {dayNames[idx]}
                  </p>
                  <p
                    className={`text-xs ${isToday ? "text-blue-500 font-medium" : "text-gray-500"}`}
                  >
                    {displayDate}
                  </p>
                </div>
                ...
                <div
                  className={`min-h-[120px] rounded-xl border-2 ${classesToday.length > 0 ? "border-blue-300 bg-blue-50" : "border-dashed border-gray-200"} p-2 flex flex-col gap-2`}
                >
                  {classesToday.length === 0 && (
                    <span className="text-sm text-gray-400 mt-4">Trống</span>
                  )}
                  {classesToday.map((lh, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className={`bg-white p-2 rounded-lg shadow-sm text-left border relative ${isOverdue && lh.trang_thai_buoi_hoc === "chua_hoc" ? "border-orange-300 bg-orange-50" : "border-blue-100"}`}
                      >
                        <p className="text-xs font-bold text-blue-700 truncate">
                          {lh.ten_lop || `Lớp #${lh.lop_hoc_id}`}
                        </p>
                        <p className="text-[10px] text-gray-500 flex items-center gap-1 mt-1 font-medium">
                          <Clock size={10} /> {lh.gio_bat_dau.substring(0, 5)} -{" "}
                          {lh.gio_ket_thuc.substring(0, 5)}
                        </p>
                        ...
                        <div className="mt-2 pt-2 border-t border-gray-100">
                          {lh.trang_thai_buoi_hoc === "chua_hoc" ? (
                            isOverdue ? (
                              <button
                                onClick={() =>
                                  handleChotCaClick(lh.lich_hoc_id)
                                }
                                className="w-full text-[10px] bg-orange-500 text-white py-1.5 rounded hover:bg-orange-600 font-bold transition-colors shadow-sm"
                              >
                                Quá hạn - Chốt ngay
                              </button>
                            ) : isReady ? (
                              <button
                                ...
                                className="w-full text-[10px] bg-blue-600 text-white py-1.5 rounded hover:bg-blue-700 font-medium transition-colors"
                              >
                                Chốt ca dạy
                              </button>
                            ) : (
                              <button
                                disabled
                                className="w-full text-[10px] bg-gray-100 text-gray-400 py-1.5 rounded font-medium cursor-not-allowed border border-gray-200"
                                title="Chưa đến giờ dạy"
                              >
                                Chưa đến giờ
                              </button>
                            )
                            ...
                            <div className="flex flex-col gap-1.5">
                              <span
                                className={`text-[10px] px-2 py-0.5 rounded-full ${getStatusInfo(lh.trang_thai_buoi_hoc, lh.ngay_hoc, lh.gio_ket_thuc, user?.role).color} block text-center font-semibold`}
                              >
                                Đã hoàn thành
                              </span>
                              <button
                                onClick={() =>
                                  handleOpenDiemDanh(lh.lich_hoc_id)
                                }
                                className="w-full text-[10px] bg-emerald-600 text-white py-1.5 rounded hover:bg-emerald-700 flex items-center justify-center gap-1 shadow-sm transition-colors"
                              >
            ...
            return (
              <div key={dateStr} className="text-center">
                ...
                >
                  ...
                  {classesToday.map((lh, i) => {
                    const currentStatus =
                      lh.trang_thai || lh.trang_thai_buoi_hoc;
                    ...
                    return (
                      <div
                        key={i}
                        className="bg-white p-2 rounded-lg shadow-sm text-left border border-blue-100 relative"
                      >
                        <p className="text-xs font-bold text-blue-700 truncate">
                          {lh.ten_lop || `Lớp #${lh.lop_hoc_id}`}
                        </p>
                        ...
                        <div className="mt-2 pt-2 border-t border-gray-100 flex flex-col gap-1.5">
                          <span
                            className={`text-[10px] px-2 py-0.5 rounded-full ${status.color} block text-center font-semibold`}
                          >
                            {status.label}
                          </span>
                          ...
                          {currentStatus === "da_hoc" && (
                            <button
                              onClick={() => handleOpenDiemDanh(lh.lich_hoc_id)}
                              className="w-full text-[10px] bg-emerald-50 text-emerald-700 py-1.5 rounded border border-emerald-200 hover:bg-emerald-100 flex items-center justify-center gap-1 font-bold transition-colors"
                            >
              ...
              <form onSubmit={handleSubmit} className="p-5 space-y-4">
                {!editingId && (
                  <div className="flex gap-4 p-1 bg-gray-100 rounded-lg">
                    <label
                      className={`flex-1 flex items-center justify-center gap-2 cursor-pointer py-2 rounded-md transition-colors ${!formData.tao_chu_ky ? "bg-white shadow-sm font-bold text-blue-600" : "text-gray-600"}`}
                    >
                      <input
                        type="radio"
                        className="hidden"
                        checked={!formData.tao_chu_ky}
                        onChange={() =>
                          setFormData({ ...formData, tao_chu_ky: false })
                        }
                      />
                      Tạo 1 buổi đơn
                    </label>
                    <label
                      className={`flex-1 flex items-center justify-center gap-2 cursor-pointer py-2 rounded-md transition-colors ${formData.tao_chu_ky ? "bg-white shadow-sm font-bold text-emerald-600" : "text-gray-600"}`}
                    >
                      <input
                        type="radio"
                        className="hidden"
                        checked={formData.tao_chu_ky}
                        onChange={() =>
                          setFormData({ ...formData, tao_chu_ky: true })
                        }
                      />
                      Tạo định kỳ (Chu kỳ)
                    </label>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, lop_hoc_id: e.target.value })
                    }
                    className="w-full p-2.5 border border-gray-300 rounded-lg text-gray-900 bg-white focus:ring-2 focus:ring-blue-500"
                    required
                  >
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, ngay_hoc: e.target.value })
                        }
                        className="w-full p-2.5 border border-gray-300 rounded-lg text-gray-900"
                        required={!formData.tao_chu_ky}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {weekDaysOptions.map((day) => {
                          const isSelected = formData.ngay_trong_tuan.includes(
                            day.value,
                          );
                          return (
                            <label
                              key={day.value}
                              className={`px-3 py-1.5 border rounded-lg cursor-pointer text-sm font-medium transition-colors ${isSelected ? "bg-emerald-50 border-emerald-500 text-emerald-700 shadow-sm" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"}`}
                            >
                              <input
                                type="checkbox"
                                className="hidden"
                                ...
                                onChange={(e) => {
                                  ...
                                  setFormData({
                                    ...formData,
                                    ngay_trong_tuan: newDays,
                                    thoi_gian_tung_ngay: newTimes,
                                  });
                                }}
                              />
                              {day.label}
                    ...
                    {formData.ngay_trong_tuan.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-emerald-200 space-y-3">
                        <p className="text-sm font-bold text-gray-800">
                          Cài đặt giờ học riêng cho từng ngày:
                        </p>
                        {formData.ngay_trong_tuan.map((dayVal) => {
                          const dayLabel = weekDaysOptions.find(
                            (opt) => opt.value === dayVal,
                          )?.label;
                          const timeData = formData.thoi_gian_tung_ngay[
                            dayVal
                          ...
                          return (
                            ...
                            >
                              <span className="w-20 font-bold text-emerald-700">
                                {dayLabel}
                              </span>
                        ...
                        <p className="text-xs text-gray-500 italic mt-2">
                          *Hệ thống sẽ tự lặp lịch này với các khung giờ tương
                          ứng cho đến khi đủ tổng số buổi của lớp.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex gap-3 pt-5 border-t mt-5 sticky bottom-0 bg-white">
                  ...
                  <button
                    type="submit"
                    className={`flex-1 py-2.5 font-medium rounded-lg text-white ${formData.tao_chu_ky ? "bg-emerald-600 hover:bg-emerald-700" : "bg-blue-600 hover:bg-blue-700"}`}
                  >
                    {editingId
                      ? "Lưu thay đổi"
                      : formData.tao_chu_ky
          ...
          <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[55] px-4">
            <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              <div className="flex justify-between items-center p-5 border-b bg-gray-50">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Buổi học ID: #{selectedLichHocId}
                  </p>
                </div>
                <button
                  onClick={() => setShowDiemDanhModal(false)}
                    ...
                    {diemDanhList.map((hs, index) => (
                      ...
                      >
                        <p className="font-bold text-gray-800 mb-4 text-lg border-b pb-2 flex items-center gap-2">
                          <span className="bg-gray-100 text-gray-600 w-6 h-6 flex items-center justify-center rounded-full text-sm">
                            {index + 1}
                          </span>
                          {hs.ten_hoc_sinh}
                        </p>
                        ...
                        <div className="flex flex-col md:flex-row gap-6">
                          <div className="flex flex-wrap gap-4 flex-shrink-0">
                            <label
                              className={`flex items-center gap-2 ${isReadOnly ? "cursor-default" : "cursor-pointer"} p-2 rounded-lg border ${hs.tinh_trang === "co_mat" ? "border-emerald-500 bg-emerald-50" : "border-gray-200"}`}
                            >
                              <input
                                type="radio"
                                name={`status_${hs.hoc_sinh_id}`}
                                value="co_mat"
                              ...
                              />
                              <span
                                className={`text-sm font-bold ${hs.tinh_trang === "co_mat" ? "text-emerald-700" : "text-gray-600"}`}
                              >
                                Có mặt
                              </span>
                            </label>
                            ...
                            <label
                              className={`flex items-center gap-2 ${isReadOnly ? "cursor-default" : "cursor-pointer"} p-2 rounded-lg border ${hs.tinh_trang === "vang" ? "border-red-500 bg-red-50" : "border-gray-200"}`}
                            >
                              <input
                                type="radio"
                                name={`status_${hs.hoc_sinh_id}`}
                              ...
                              />
                              <span
                                className={`text-sm font-bold ${hs.tinh_trang === "vang" ? "text-red-700" : "text-gray-600"}`}
                              >
                                Vắng
                              </span>
                            </label>
                            ...
                            <label
                              className={`flex items-center gap-2 ${isReadOnly ? "cursor-default" : "cursor-pointer"} p-2 rounded-lg border ${hs.tinh_trang === "vang_co_phep" ? "border-orange-500 bg-orange-50" : "border-gray-200"}`}
                            >
                              <input
                                type="radio"
                                name={`status_${hs.hoc_sinh_id}`}
                              ...
                              />
                              <span
                                className={`text-sm font-bold ${hs.tinh_trang === "vang_co_phep" ? "text-orange-700" : "text-gray-600"}`}
                              >
                                Vắng có phép
                              </span>
                            </label>
                          ...
                          <div className="flex-1">
                            <input
                              ...
                              placeholder={
                                isReadOnly
                                  ? "Không có ghi chú"
                                  : "Nhập ghi chú nhận xét (tùy chọn)..."
                              }
                              value={hs.ghi_chu || ""}
                              onChange={(e) =>
                                updateDiemDanh(
              ...
              <div className="flex gap-4 p-5 border-t bg-white shadow-[0_-4px_10px_rgba(0,0,0,0.03)]">
                <button
                  onClick={() => setShowDiemDanhModal(false)}
                  className={`py-3 border-2 border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-colors ${isReadOnly ? "w-full" : "flex-1"}`}
                >
                  Đóng lại
                </button>
                ...
                {!isReadOnly && (
                  <button
                    ...
                    disabled={loadingDiemDanh || diemDanhList.length === 0}
                    className="flex-[2] py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-all disabled:bg-emerald-300 shadow-lg shadow-emerald-200"
                  >
                    Lưu Phiếu Điểm Danh
                  </button>
                )}
  ...
  const filteredAdminLichHocs = lichHocs.filter((lh) => {
    ...
    const tenLop = lh.ten_lop
      ? lh.ten_lop.toLowerCase()
      : `lớp #${lh.lop_hoc_id}`;
    ...
    const tenGiaSu = lh.ten_gia_su
      ? lh.ten_gia_su.toLowerCase()
      : "đang cập nhật";
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex justify-between items-center pb-3 border-b border-gray-100">
                  ...
                  <span className="font-medium text-gray-800 flex items-center gap-1">
                    ...
                    {detailModal.data.ten_gia_su || "Đang cập nhật"}
                  </span>
                </div>
                <div className="flex justify-between items-center pb-3 border-b border-gray-100">
                  ...
                  <div className="text-right">
                    <span className="font-bold text-emerald-600 block">
                      {getThuString(detailModal.data.ngay_hoc)},{" "}
                      {new Date(detailModal.data.ngay_hoc).toLocaleDateString(
                        "vi-VN",
                      )}
                    </span>
                    <span className="font-medium text-gray-700 flex items-center justify-end gap-1 mt-0.5">
                      <Clock size={14} className="text-amber-500" />
                      {detailModal.data.gio_bat_dau.substring(0, 5)} -{" "}
                      {detailModal.data.gio_ket_thuc.substring(0, 5)}
                    </span>
                  </div>
                </div>
                <div className="flex justify-between items-center pb-1">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold ${getStatusInfo(detailModal.data.trang_thai, detailModal.data.ngay_hoc, detailModal.data.gio_ket_thuc, user?.role).color}`}
                  >
                    {
                      getStatusInfo(
                        detailModal.data.trang_thai,
                        detailModal.data.ngay_hoc,
                        detailModal.data.gio_ket_thuc,
                        user?.role,
                      ).label
                    }
                  </span>
                            ...
                            >
                              ...
                              <div className="flex-1 flex gap-2 items-center">
                                <input
                                  ...
                                  required
                                  value={timeData.gio_bat_dau}
                                  className="w-full text-sm p-2 border border-gray-300 rounded focus:ring-emerald-500 focus:border-emerald-500 text-gray-900 bg-white"
                                  onChange={(e) =>
                                    setFormData({
                                      ...formData,
                                      thoi_gian_tung_ngay: {
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4 border-b pb-4">
        ...
        <div className="flex flex-col md:flex-row items-center gap-3 w-full xl:w-auto">
          ...
          <button
            ...
            className="bg-blue-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-blue-700 w-full md:w-auto justify-center font-medium shadow-md shrink-0"
          >
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          ...
          <tbody>
            {filteredAdminLichHocs.length === 0 ? (
              ...
              filteredAdminLichHocs
                .sort((a, b) => new Date(b.ngay_hoc) - new Date(a.ngay_hoc))
                .map((lh) => {
                  const status = getStatusInfo(
                    lh.trang_thai,
                    lh.ngay_hoc,
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 text-sm text-gray-600 flex items-center gap-1 mt-1">
                        <Clock size={14} className="text-gray-400" />{" "}
                        {lh.gio_bat_dau.substring(0, 5)} -{" "}
                        {lh.gio_ket_thuc.substring(0, 5)}
                      </td>
                      <td className="p-3">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-semibold shadow-sm whitespace-nowrap ${status.color}`}
                        >
                          {status.label}
                        </span>
                      </td>
                      <td className="p-3 text-center relative flex justify-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            setDetailModal({ show: true, data: lh })
                          }
                          className="px-3 py-1.5 bg-white border border-gray-300 text-gray-700 hover:bg-gray-100 rounded-lg text-xs font-bold transition-colors flex items-center gap-1 shadow-sm"
                        >
                        ...
                        {lh.trang_thai === "da_hoc" ? (
                          <button
                            onClick={() => handleOpenDiemDanh(lh.lich_hoc_id)}
                            className="px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-lg hover:bg-emerald-100 transition-colors flex items-center gap-1 font-semibold text-xs border border-emerald-200"
                          >
                          ...
                          <div className="flex items-center gap-2">
                            {!isOverdue && (
                              <button
                                ...
                                className="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors tooltip border border-blue-100"
                                title="Sửa lịch"
                              >
                                <Edit2 size={16} />
                              </button>
                            ...
                            <button
                              onClick={() => handleDeleteClick(lh.lich_hoc_id)}
                              className="p-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors tooltip border border-red-100"
                              title="Xóa lịch"
                            >
                              <Trash2 size={16} />
                            </button>
