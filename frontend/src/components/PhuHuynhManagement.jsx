...
export default function PhuHuynhManagement() {
  ...
  const API_URL = 'http://localhost:8080/QuanLyTrungTamGiaSu/backend/public/api'

  // Lấy số học sinh thực tế của phụ huynh
  const fetchStudentCount = async (parentId) => {
    try {
      const response = await fetch(`${API_URL}/phuhuynh/${parentId}`, {
        headers: {
  ...
  const fetchParents = async (page = 1, searchTerm = '') => {
    try {
      setLoading(true)
      let url = `${API_URL}/phuhuynh?page=${page}&limit=10`
      if (searchTerm) {
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/phuhuynh`, {
        method: 'POST',
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    setModalLoading(true)
    try {
      const response = await fetch(`${API_URL}/phuhuynh/${editModal.data.phu_huynh_id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
  ...
  const handleDelete = async (parent) => {
    ...
    if (window.confirm(`Bạn có chắc chắn muốn xóa phụ huynh "${parent.ho_ten}"? Thao tác này không thể hoàn tác.`)) {
      try {
        setLoading(true)
        const response = await fetch(`${API_URL}/phuhuynh/${parent.phu_huynh_id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
              ...
              <tbody className="divide-y divide-gray-200">
                {parents.map((parent, index) => (
                  <tr key={parent.phu_huynh_id} className="hover:bg-blue-50 transition-colors duration-200">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm text-gray-700 break-words">{parent.email}</p>
                    </td>
                    <td className="px-6 py-4">
