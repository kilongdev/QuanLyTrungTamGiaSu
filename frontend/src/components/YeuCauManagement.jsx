...
import { Plus, Settings, X, Search, Trash2, Edit2, MessageSquare } from 'lucide-react'
...
export default function YeuCauManagement({ user }) {
  ...
  const getPhanLoaiLabel = (phanLoai) => {
    const labels = { 'mo_lop': 'Mở lớp mới', 'huy_lop': 'Hủy lớp', 'nghi_day': 'Xin nghỉ dạy', 'nghi_hoc': 'Xin nghỉ học', 'khac':  'Yêu cầu khác' }
    return labels[phanLoai] || phanLoai
  }

  const getTrangThaiLabel = (trangThai) => {
    const labels = { 'cho_duyet': 'Chờ xử lý', 'dang_xu_ly': 'Đang xử lý', 'da_duyet': 'Đã duyệt', 'tu_choi': 'Từ chối', 'da_hoan_thanh': 'Đã hoàn thành' }
  ...
  const filteredYeuCaus = yeuCaus.filter(yc => 
    yc.tieu_de?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    yc.loai_nguoi_tao?.toLowerCase().includes(searchTerm.toLowerCase())
  )
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-800">Quản lý Yêu cầu</h2>
          <p className="text-sm text-gray-500">Tổng số: {yeuCaus.length} yêu cầu</p>
        </div>
        {user?.role !== 'admin' && (
        <button
      ...
      {filteredYeuCaus.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm p-12 text-center">
          <p className="text-gray-500">{searchTerm ? 'Không tìm thấy yêu cầu nào' : 'Chưa có yêu cầu nào'}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredYeuCaus.map((yc) => (
            <div key={yc.yeu_cau_id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow relative group">
              ...
              <div className="absolute top-2 right-2">
                <div className="relative">
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      setShowSettings(showSettings === yc.yeu_cau_id ? null : yc.yeu_cau_id)
                    }}
                  ...
                  {showSettings === yc.yeu_cau_id && (
                    <div className="absolute top-8 right-0 bg-white border border-gray-200 rounded-lg shadow-lg z-10 min-w-[140px]">
                      {(user?.role === 'admin' || (user?.role !== 'admin' && yc.trang_thai === 'cho_duyet')) && (
                          <button
                            onClick={() => { handleEdit(yc); setShowSettings(null); }}
                          ...
                          >
                            <Edit2 size={12} /> 
                            {user?.role === 'admin' ? 'Xử lý / Cập nhật' : 'Chỉnh sửa nội dung'}
                          </button>
                      )}

                      <button
                        onClick={() => { handleDelete(yc.yeu_cau_id); setShowSettings(null); }}
              ...
              <div className="p-4">
                ...
                <h3 className="font-semibold text-gray-800 text-base mb-1 pr-6 line-clamp-1" title={yc.tieu_de}>
                  {yc.tieu_de}
                </h3>
                ...
                <div className="space-y-2 mb-3">
                  <div className="text-sm text-gray-600 flex justify-between">
                    <span>Phân loại:</span>
                    <span className="font-medium text-gray-900">{getPhanLoaiLabel(yc.phan_loai)}</span>
                  </div>
                  <div className="text-sm text-gray-600 flex justify-between">
                    <span>Ngày gửi:</span>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => setFormData({ ...formData, loai_nguoi_tao: e.target.value })}
                                disabled={true}
              ...
              {(user?.role === 'admin' && editingId) && (
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 space-y-3">
                      <div>
                          <p className="text-sm font-medium text-gray-700">Tiêu đề yêu cầu:</p>
                          <p className="text-sm text-gray-900 font-semibold">{formData.tieu_de}</p>
                      </div>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Ghi chú của Admin</label>
                        <textarea
                            value={formData.ghi_chu_xu_ly}
                            onChange={(e) => setFormData({ ...formData, ghi_chu_xu_ly: e.target.value })}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900 bg-white focus:ring-2 focus:ring-blue-500"
                            rows="2"
              ...
              <div className="flex gap-3 pt-4">
                ...
                >
                  {editingId ? (user?.role === 'admin' ? 'Lưu trạng thái' : 'Cập nhật nội dung') : 'Gửi yêu cầu'}
                </button>
              </div>
            </form>
          </div>
