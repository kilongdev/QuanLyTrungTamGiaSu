...
export default function HocSinhManagement() {
  ...
  const fetchStudents = async (page = 1, searchTerm = "") => {
    try {
      setLoading(true);
      const response = await hocSinhAPI.getAll({
        page,
        limit: 10,
        search: searchTerm,
      });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (modalMode === "edit") {
        const response = await hocSinhAPI.update(
          currentStudent.hoc_sinh_id,
          formData,
        );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 pb-4 border-b border-gray-200">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Quản lý hồ sơ và thông tin học tập của học sinh
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-xl p-5 shadow-lg relative overflow-hidden">
          ...
          <Users
            className="absolute right-4 bottom-4 text-white opacity-20"
            size={48}
          />
        </div>
        ...
        {students.length === 0 && !loading ? (
          <div className="p-16 text-center">
            ...
            <p className="text-gray-400 text-sm mt-2">
              Hãy thêm học sinh mới hoặc thay đổi bộ lọc tìm kiếm
            </p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        <div
                          className={`${getAvatarColor(student.hoc_sinh_id)} w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm`}
                        >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="inline-flex items-center justify-center bg-blue-100 text-blue-700 text-xs font-bold px-3 py-1 rounded-full border border-blue-200">
                        Khối {student.khoi_lop || "N/A"}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex justify-center gap-2">
                        <button
                          onClick={() => handleViewDetail(student.hoc_sinh_id)}
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full transform transition-all">
            ...
            <form onSubmit={handleFormSubmit}>
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-2">
                    <BookUser size={14} /> Phụ huynh{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <select
                    value={formData.phu_huynh_id}
                    onChange={(e) =>
                      setFormData({ ...formData, phu_huynh_id: e.target.value })
                    }
              ...
              <>
                ...
                <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-6 flex justify-between items-start rounded-t-xl">
                  <div className="flex items-center gap-4">
                    <div
                      className={`${getAvatarColor(detailModal.data.hoc_sinh_id)} w-14 h-14 rounded-full flex items-center justify-center font-bold text-xl border-2 border-white/50`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-blue-100">
                        ID: {detailModal.data.hoc_sinh_id} • Khối:{" "}
                        {detailModal.data.khoi_lop}
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="p-6 space-y-6 overflow-y-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-gray-900">
                            {detailModal.data.phu_huynh_ten || "Chưa cập nhật"}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-gray-900">
                            {detailModal.data.phu_huynh_sdt || "Chưa cập nhật"}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg col-span-full">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-gray-900">
                            {detailModal.data.phu_huynh_email ||
                              "Chưa cập nhật"}
                          </p>
                        </div>
                      </div>
                  ...
                  <div>
                    ...
                    {detailModal.data.lop_hoc &&
                    detailModal.data.lop_hoc.length > 0 ? (
                      ...
                      <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg">
                        <p className="text-yellow-800 text-sm">
                          ℹ️ Học sinh này chưa đăng ký lớp học nào.
                        </p>
                      </div>
                    )}
                  </div>
