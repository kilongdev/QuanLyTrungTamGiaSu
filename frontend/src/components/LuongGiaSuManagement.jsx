...
export default function LuongGiaSuManagement({ user }) {
  ...
  const fetchGiaSuList = async () => {
    try {
      const response = await fetch('http://localhost/QuanLyTrungTamGiaSu/backend/public/api.php?route=/giasu&limit=100', {
  ...
  const fetchLopHocList = async () => {
    try {
      const response = await fetch('http://localhost/QuanLyTrungTamGiaSu/backend/public/api.php?route=/lophoc', {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      })
      const data = await response.json()
