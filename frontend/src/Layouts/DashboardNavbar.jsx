...
export default function DashboardNavbar({ user, onLogout, pageTitle, sidebarCollapsed, onToggleSidebar, onEditProfile, onChangePassword }) {
  ...
  const getUserType = () => {
    switch (user?.role) {
      ...
      case 'gia_su': return 'gia_su'
      case 'phu_huynh': return 'phu_huynh'
  ...
  const fetchNotifications = async () => {
    ...
    try {
      setLoading(true)
      const response = await thongBaoAPI.getMyNotifications(user.id, getUserType())
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      setNotifications(prev => prev.map(n => ({ ...n, trang_thai: 'da_doc' })))
      setUnreadCount(0)
    } catch (error) {
                    ...
                    notifications.map((notification) => (
                      <div 
                        ...
                        className={`px-4 py-3 border-b border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors ${
                          notification.trang_thai === 'chua_doc' ? 'bg-blue-50' : ''
                        }`}
                        onClick={() => handleNotificationClick(notification)}
                      >
                        <div className="flex items-start gap-3">
                          <div className={`w-2 h-2 mt-2 rounded-full flex-shrink-0 ${
                            notification.trang_thai === 'chua_doc' ? 'bg-blue-500' : 'bg-gray-300'
                          }`}></div>
          ...
          >
            ...
            <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-5">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-lg">{selectedNotification.tieu_de}</h3>
                    <span className={`inline-block mt-1 px-2 py-0.5 rounded text-xs ${
                      selectedNotification.trang_thai === 'chua_doc' 
                        ? 'bg-yellow-400 text-yellow-900' 
                        : 'bg-green-400 text-green-900'
                    }`}>
                      {selectedNotification.trang_thai === 'chua_doc' ? 'Chưa đọc' : 'Đã đọc'}
                    </span>
                  </div>
                </div>
                <button 
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-gray-800 leading-relaxed">{selectedNotification.noi_dung}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-3 border-t border-gray-100">
                <div className="flex items-center gap-2 text-gray-600">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{formatTime(selectedNotification.ngay_tao)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-gray-600">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium capitalize">
                      {selectedNotification.loai_thong_bao === 'he_thong' ? 'Hệ thống' : 
                       selectedNotification.loai_thong_bao === 'dang_ky' ? 'Đăng ký' :
                       selectedNotification.loai_thong_bao === 'yeu_cau' ? 'Yêu cầu' : 
