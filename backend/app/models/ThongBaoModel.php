...
class ThongBaoModel extends BaseModel
{
     ...
     */
    public static function guiThongBao(
        $nguoiNhanId,
        $loaiNguoiNhan,
        $tieuDe,
        $noiDung,
        ...
        $nguoiGuiId = 0,
        $loaiNguoiGui = 'admin' // ✅ FIX: đổi default từ 'system' -> 'admin'
    ) {
        $model = new self();

        // ✅ Chuẩn hóa dữ liệu để tránh lỗi ENUM
        $loaiNguoiGui = trim(strtolower($loaiNguoiGui));
        $loaiNguoiNhan = trim(strtolower($loaiNguoiNhan));

        $validTypes = ['admin', 'gia_su', 'phu_huynh'];

        if (!in_array($loaiNguoiGui, $validTypes)) {
            $loaiNguoiGui = 'admin';
        }

        if (!in_array($loaiNguoiNhan, $validTypes)) {
            $loaiNguoiNhan = 'phu_huynh';
        }
    ...
    public function create($data)
    {
        try {
            $sql = "INSERT INTO {$this->table} 
                (nguoi_gui_id, loai_nguoi_gui, nguoi_nhan_id, loai_nguoi_nhan, loai_thong_bao, tieu_de, noi_dung, da_doc, ngay_tao) 
            ...
            // ✅ Không làm crash hệ thống chính
            error_log("ThongBao error: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function markAllAsRead($userId, $userType)
    {
        $sql = "UPDATE {$this->table} 
                SET da_doc = 1 
                WHERE nguoi_nhan_id = ? AND loai_nguoi_nhan = ? AND da_doc = 0";
