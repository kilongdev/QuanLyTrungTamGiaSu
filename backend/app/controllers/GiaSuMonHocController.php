...
final class GiaSuMonHocController extends BaseController
{
    private GiaSuMonHoc $model;

    public function __construct()
    {
        $this->model = new GiaSuMonHoc();
    }
    ...
    public function show(int $id): void
    {
        ...
        if ($row === null) {
            $this->notFound('Liên kết gia sư - môn học không tồn tại');
            return;
        }
    ...
    {
        $data = $this->inputJson();
        $missing = $this->requireFields($data, ['gia_su_id', 'mon_hoc_id']);
        if ($missing !== null) {
